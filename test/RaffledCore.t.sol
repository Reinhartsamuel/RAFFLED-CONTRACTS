...
contract RaffledCoreTest is Test {
    ...
    function _createERC20RaffleWithParams(uint256 ticketPrice, uint256 maxCap, uint256 duration) internal returns (uint256) {
        vm.prank(HOST);
        IERC20(address(prizeToken)).approve(address(mgr), PRIZE_AMT);
        vm.prank(HOST);
    ...
    function _createERC721RaffleWithParams(uint256 ticketPrice, uint256 maxCap, uint256 duration) internal returns (uint256) {
        vm.prank(HOST);
    ...
    function _signFreeEntry(uint256 raffleId, address user) internal view returns (bytes memory) {
        bytes32 structHash = keccak256(abi.encode(
            keccak256("FreeEntry(uint256 raffleId,address user)"),
            raffleId,
            user
        ));
