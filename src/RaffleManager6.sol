...
///
/// @dev    Security-hardened fork of RaffleManager5. Changes:
///         - Finding 1: manualFulfillWinner removed entirely
///         - Finding 2: verifyAndClaim is internal (via FreeEntryVerifier2)
///         - Finding 3: checkUpkeep uses bounded pagination with wrap-around
///         - Finding 4: ERC-721 prizes use pull-based distribution with claim timeout
///         - Finding 5: Single ownership model (no verifierOwner divergence)
///         - Finding 6: Permissionless emergencyFinalize after VRF timeout
///         - R2-1: emergencyFinalize skips prize return for underfilled raffles
///         - R2-2: performUpkeep validates raffleId bounds
///         - R2-3: ERC-721 claim timeout (30 days) with host reclaim
///         - R2-4: checkUpkeep wraps scan to catch skipped raffles
///         - Lead fix: setMinDuration enforces 2-hour floor
///         - Lead fix: Zero-participant path sets underfilled = true
///         - Lead fix: uint48 expiry overflow check in _initRaffle
///         - V6 ARCHITECTURAL UPGRADE 1: Explicit ERC721ClaimStatus state machine
///         - V6 ARCHITECTURAL UPGRADE 2: TicketRange storage model (O(unique buyers) instead of O(total tickets))
///         - V6 ARCHITECTURAL UPGRADE 3: Pull-based participant refunds on emergencyFinalize
///
///   Storage packing: 5 EVM slots per raffle. 2 EVM slots per unique buyer ticket range.
contract RaffleManager6 is
    ...
    FreeEntryVerifier2
{
    ...
    /// @notice Stores the winner for ERC-721 full-fill raffles awaiting claim.
    mapping(uint256 => address) public pendingERC721Winner;

    /// @notice Timestamp when ERC-721 prize became claimable (for timeout).
    mapping(uint256 => uint48) private erc721PrizeClaimableAt;

    uint256 public raffleCount;

    mapping(uint256 => ERC721ClaimStatus) public claimStatus;
    ...
    function emergencyFinalize(uint256 _raffleId) external nonReentrant {
        ...
        claimStatus[_raffleId] = ERC721ClaimStatus.CANCELLED;
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] calldata _randomWords) internal override nonReentrant {
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            if (winningTicket <= ranges[i].endTicket) {
                winner = ranges[i].owner;
                break;
            }
        }
        ...
        if (raffle.prizeType == PrizeType.ERC721 && !raffle.underfilled) {
            ...
            claimStatus[raffleId] = ERC721ClaimStatus.CLAIMABLE;
    ...
    function claimERC721Prize(uint256 _raffleId) external nonReentrant {
        if (claimStatus[_raffleId] != ERC721ClaimStatus.CLAIMABLE) revert NoPendingPrize();
        if (msg.sender != pendingERC721Winner[_raffleId]) revert NotWinner();

        claimStatus[_raffleId] = ERC721ClaimStatus.CLAIMED;
    ...
    function hostReclaimUnclaimed(uint256 _raffleId) external nonReentrant {
        ...
        if (block.timestamp < erc721PrizeClaimableAt[_raffleId] + ERC721_CLAIM_TIMEOUT) {
            revert ClaimTimeoutNotReached();
        }

        claimStatus[_raffleId] = ERC721ClaimStatus.EXPIRED;
        delete pendingERC721Winner[_raffleId];
        delete erc721PrizeClaimableAt[_raffleId];
        ...
        if (paymentPool > 0) {
            IERC20(paymentToken).safeTransfer(raffle.host, paymentPool - paymentFee);
        }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // Participant Refunds (Pull-based)
    // ──────────────────────────────────────────────────────────────────────
    ...
    function claimRefund(uint256 _raffleId) external nonReentrant {
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            if (ranges[i].owner == msg.sender && ranges[i].amountPaid > 0) {
                refundAmount += ranges[i].amountPaid;
                ranges[i].amountPaid = 0; // Mark as refunded
            }
        }
    ...
    function getTicketRange(uint256 _raffleId, uint256 _index)
        external
        view
        returns (address owner, uint256 endTicket, uint256 amountPaid)
    {
        TicketRange storage range = ticketRanges[_raffleId][_index];
        return (range.owner, range.endTicket, range.amountPaid);
    }
    ...
    function _addTickets(uint256 _raffleId, address _buyer, uint256 _ticketCount, uint256 _amountPaid) internal {
        ...
        if (len > 0 && ranges[len - 1].owner == _buyer) {
            ...
            ranges[len - 1].amountPaid += uint96(_amountPaid);
        } else {
            ranges.push(TicketRange({owner: _buyer, endTicket: newTotal, amountPaid: uint96(_amountPaid)}));
        }
